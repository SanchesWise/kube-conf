...
# Распространение на ноды кластера.
# nodes=("10.10.2.89" "10.10.2.101" "10.10.2.102" "10.10.2.103" "10.10.2.104" "10.10.2.105" "10.10.2.106" "10.10.2.107" "10.10.2.108" "10.10.2.109")
# for node in "${nodes[@]}"; do
#     echo "Processing $node...";
#     scp ~/.bashrc ccsfarm@$node:~/.bashrc;
#     ssh ccsfarm@$node "kubectl completion bash > ~/.kubectl_completion";
# done
...
if [ $(id -un) = root ]; then
  ...
  PS1="┌ [${Cyan}\A${Color_Off}][${BIGreen}\u${Color_Off}]@[${Yellow}\H${Color_Off}][${BICyan}\w${Color_Off}]${show_git}\n└─ \$ "
fi
...
filepath="${HOME}/vault_env"
    if [ -f "$filepath" ]; then
        source "$filepath"
    fi
fi
# Проверяем наличие файла дополнений и подключаем его
if [ -f ~/.kubectl_completion ]; then
    source ~/.kubectl_completion
    # Настройка дополнений для короткого алиаса 'k'
    alias k='kubectl'
    complete -o default -F __start_kubectl k
else
    # Если файла нет (например, на новой ноде), генерируем на лету (фолбэк)
